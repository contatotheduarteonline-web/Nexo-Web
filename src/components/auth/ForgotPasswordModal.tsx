...
export const ForgotPasswordModal: React.FC<ForgotPasswordModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-4 backdrop-blur-xs">
      <div className="w-full max-w-md rounded-2xl border border-slate-200 bg-white p-6 shadow-2xl dark:border-[#27272A] dark:bg-[#0F172A]">
        <div className="flex items-center justify-between border-b border-slate-100 pb-3 dark:border-[#27272A]">
          <div className="flex items-center gap-2.5">
            <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-amber-500/15 text-[#F59E0B] border border-amber-500/30">
          ...
          <button
            onClick={handleClose}
            className="rounded-lg p-1 text-slate-500 hover:bg-slate-100 dark:hover:bg-[#1E293B] dark:text-slate-400"
          >
            <X className="h-5 w-5" />
          </button>
        </div>
          ...
          <form onSubmit={handleRequestReset} className="mt-4 space-y-4">
            ...
            <div>
              ...
              <div className="relative mt-1">
                <input
                  ...
                  className="w-full rounded-xl border border-slate-200 bg-slate-50 py-2.5 pl-9 pr-3 text-xs text-[#09090B] placeholder:text-slate-400 focus:border-[#F59E0B] focus:bg-white focus:outline-hidden dark:border-[#27272A] dark:bg-[#1E293B] dark:text-white dark:focus:border-[#F59E0B]"
                />
            ...
            <div className="flex justify-end gap-2 pt-2">
              <button
                type="button"
                onClick={handleClose}
                className="rounded-xl px-4 py-2 text-xs font-semibold text-slate-600 hover:bg-slate-100 dark:text-slate-300 dark:hover:bg-[#1E293B]"
              >
                Cancelar
              </button>
              <button
                type="submit"
                disabled={loading}
                className="flex items-center gap-1.5 rounded-xl bg-gradient-to-r from-[#F59E0B] to-[#FBBF24] px-5 py-2.5 text-xs font-bold text-white shadow-md shadow-amber-500/25 transition hover:from-[#D97706] hover:to-[#F59E0B] disabled:opacity-50 cursor-pointer"
              >
        ...
        {step === "success" && (
          <div className="mt-5 space-y-4 text-center">
            ...
            <button
              type="button"
              onClick={handleClose}
              className="mt-3 w-full rounded-xl bg-gradient-to-r from-[#F59E0B] to-[#FBBF24] py-2.5 text-xs font-bold text-white shadow-lg shadow-amber-500/25 hover:from-[#D97706] hover:to-[#F59E0B] cursor-pointer"
            >
              Voltar ao Login
            </button>
          </div>
