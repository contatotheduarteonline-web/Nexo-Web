    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col justify-between gap-4 rounded-2xl border border-slate-200 bg-white p-6 shadow-xs sm:flex-row sm:items-center dark:border-slate-800 dark:bg-slate-900">
        ...
        <button
          ...
          className="flex items-center gap-2 rounded-xl bg-gradient-to-r from-[#111622] to-[#2B353D] border border-orange-500/40 px-4 py-2.5 text-xs font-bold text-[#FFA726] shadow-xs transition hover:brightness-110 disabled:opacity-50 cursor-pointer"
        >
      ...
      {aiAnalysis && (
        <div className="rounded-2xl border border-orange-500/30 bg-orange-50/60 p-6 dark:border-orange-500/30 dark:bg-orange-500/10">
          <div className="flex items-center gap-2 text-[#FF6B00] dark:text-[#FFA726]">
          ...
          <div className="mt-4 grid grid-cols-1 gap-4 md:grid-cols-2">
              ...
              <ul className="mt-2 space-y-1.5 text-xs text-slate-700 dark:text-slate-300">
                {aiAnalysis.keyActions?.map((act: string, idx: number) => (
                  <li key={idx} className="flex items-start gap-2">
                    <span className="font-bold text-[#FF6B00] dark:text-[#FFA726]">•</span>
                    <span>{act}</span>
                  </li>
                ))}
              </ul>
            ...
            <div className="rounded-xl bg-white p-4 shadow-xs dark:bg-slate-900">
              ...
              {aiAnalysis.revisionTip && (
                <div className="mt-3 rounded-lg bg-orange-50 border border-orange-500/20 p-2.5 text-[11px] font-semibold text-[#FF6B00] dark:bg-orange-950/40 dark:text-[#FFA726]">
                  🔁 {aiAnalysis.revisionTip}
                </div>
              )}
            </div>
                ...
                >
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <button
                      ...
                      className="rounded-lg bg-[#FF6B00] px-3 py-1.5 text-xs font-bold text-white shadow-xs transition hover:bg-[#E05D00]"
                    >
                      Treinar
                    </button>
                  </div>
          ...
          <div className="mt-4 space-y-3">
            {metrics.weakTopics.length === 0 ? (
              ...
              metrics.weakTopics.map((item) => (
                <div
                  key={item.topicId}
                  className="flex flex-col justify-between gap-3 rounded-xl border border-orange-100 bg-orange-50/30 p-4 sm:flex-row sm:items-center dark:border-orange-900/30 dark:bg-orange-950/20"
                >
                  <div className="space-y-1">
                    <span className="text-[10px] font-bold text-orange-700 uppercase dark:text-orange-300">
                      {item.disciplineName}
                    </span>
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      className="flex items-center gap-1 rounded-lg border border-orange-500/40 bg-orange-50 px-2.5 py-1.5 text-xs font-bold text-[#FF6B00] hover:bg-orange-100 dark:bg-orange-950/30 dark:text-[#FFA726]"
                    >
                    ...
                    <button
                      ...
                      className="flex items-center gap-1 rounded-lg bg-[#FF6B00] px-3 py-1.5 text-xs font-bold text-white shadow-xs transition hover:bg-[#E05D00]"
                    >
      ...
      {practiceQuestions.length > 0 && (
        <div className="rounded-2xl border border-orange-500/40 bg-white p-6 shadow-xs dark:border-orange-500/30 dark:bg-slate-900">
          <div className="flex items-center justify-between border-b border-slate-100 pb-4 dark:border-slate-800">
            <div>
              <span className="rounded-full bg-orange-50 px-2.5 py-0.5 text-xs font-bold text-[#FF6B00] border border-orange-500/30 dark:bg-orange-950/40 dark:text-[#FFA726]">
            ...
            {practiceQuestions.map((q, qIdx) => (
              <div key={q.id || qIdx} className="rounded-xl border border-slate-100 bg-slate-50/70 p-5 dark:border-slate-800 dark:bg-slate-800/40">
                <p className="text-xs font-semibold text-slate-800 dark:text-slate-200">
                  <span className="font-bold text-[#FF6B00] dark:text-[#FFA726]">Questão {qIdx + 1}:</span> {q.statement}
                </p>
                ...
                <div className="mt-3 space-y-2">
                  {q.options.map((opt: string, optIdx: number) => {
                    ...
                    if (isSelected) {
                      btnStyle = "border-[#FF6B00] bg-orange-50 text-[#374151] font-semibold dark:bg-orange-950/30 dark:text-white";
                    }
                    if (showResult) {
                      if (isCorrect) {
                        btnStyle = "border-[#FF6B00] bg-orange-50 text-[#FF6B00] font-bold dark:bg-orange-950/40 dark:text-[#FFA726]";
                ...
                {showExplanations && (
                  <div className="mt-3 rounded-lg bg-orange-50 border border-orange-500/20 p-3 text-[11px] text-[#374151] dark:bg-orange-950/30 dark:text-[#FFA726]">
          ...
          <div className="mt-6 flex justify-end gap-3">
            <button
              ...
              className="rounded-xl bg-[#FF6B00] px-5 py-2.5 text-xs font-bold text-white shadow-xs transition hover:bg-[#E05D00]"
            >
