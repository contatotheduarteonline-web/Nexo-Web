...
export const CreatePlanWizardModal: React.FC<CreatePlanWizardModalProps> = ({
  ...
  const handleSelectCatalogEdital = async (ed: CatalogEdital) => {
    ...
    try {
      // Editais do catálogo oficial podem trazer o snapshot completo de cargos
      // quando disponível; caso contrário, a árvore é carregada do Firestore.
      const embedded = (ed as PublishedEditalSnapshot).cargos;
  ...
  const handleSelectCatalogCargo = async (ed: CatalogEdital, cargoItem: CatalogCargo) => {
    ...
    try {
      // Prioriza o snapshot embutido; caso não exista, carrega a estrutura do Firestore
  ...
  const handleFinalSubmitCreatePlan = async (publishToCatalog: boolean = false, isDraft: boolean = false) => {
    ...
    try {
      ...
      if (isAdmin && (publishToCatalog || isDraft) && !selectedCatalogEdital) {
        ...
        // 4a. PERSISTE O EDITAL COMPLETO NO FIRESTORE COMO DRAFT.
        // O registro só será marcado como "published" depois que cargos,
        // disciplinas e tópicos forem gravados com sucesso.
        try {
          ...
          // Só depois de toda a árvore estar persistida, torna o edital público.
          ...
          setCatalogValidationErrors([
            `Falha ao salvar o edital no catálogo oficial do Firestore: ${catalogErr?.message || catalogErr}`,
          ]);
          setStep("review");
          setIsSaving(false);
          return;
